diferencia_medias_significativa <- function(datos, var_grupo, vars_resultados) {
  ...
  for (var_resultados in vars_resultados) {
    ...
    resultados <- rbind(resultados, data.frame(variable = var_resultados,
...
VIP_vars <- c(ffreedoms, accountability, corruption, trust, security, cjustice, effectiveness, police, constraints, bribery, report)
...
data_subset.df <- master_data.df %>%
  select(country, country_code, year,
         all_of(VIP_vars), 
         q50, q51, q52,
         q8b_1, q8b_2, q8b_3, q8b_4, q8b_5, q8b_6, q8b_7, q8b_8, q8b_9, q8b_10, q8b_11, q8b_12, q8b_13, q8b_14, q8b_15, q8b_16, q8b_17) %>%
  mutate(
    q52 = case_when(
      q52 == 1 ~ 4,
      q52 == 2 ~ 3,
...
diffmeans.df <- diferencia_medias_significativa(datos = data2table, var_grupo = year, vars_resultados = VIP_vars)
...
diffmeans.df <-  diffmeans.df %>%
  ...
  select(variable, label, category, diff, p_valor, significance, direction_change)

write.xlsx(as.data.frame(diffmeans.df), 
