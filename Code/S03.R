...
figure12_2.fn <- function(nchart = 12, country = mainCountry) {
  ...
  denomym <- method_data.ls[["sf"]] %>%
    mutate(Country = if_else(Country %in% "The Bahamas", "Bahamas", Country)) %>%
    filter(Country %in% mainCountry) 
  ...
  if (mainCountry == "Paraguay" | mainCountry == "United States") {
    ...
    label <- c(paste0("<span style='color:#003b8a;font-size:4.217518mm'>", '**',victims$victim*100, "%",'**',"</span> <br> <span style='color:#222221;font-size:3.514598mm'> of ", denomym$Nationality ,"s","<br>were victims <br>of a crime in <br> the last 12 <br> months"),
               paste0("<span style='color:#003b8a;font-size:4.217518mm'>", '**',report$report*100, "%",'**',"</span> <br> <span style='color:#222221;font-size:3.514598mm'> of crime <br> victims reported <br> the crime"),
               paste0("<span style='color:#fa4d57;font-size:4.217518mm'>", '**',report$non_report*100, "%",'**',"</span> <br> <span style='color:#222221;font-size:3.514598mm'> of crime <br> victims did <br> not report <br>the crime"),
               paste0("<span style='color:#003b8a;font-size:4.217518mm'>", '**',fill_report$fill_report*100, "%",'**',"</span> <br> <span style='color:#222221;font-size:3.514598mm'> of cases <br> ended in <br> prosecution"),
               paste0("<span style='color:#fa4d57;font-size:4.217518mm'> ", '**',fill_report$non_fill_report*100, "%",'**',"</span> <br> <span style='color:#222221;font-size:3.514598mm'> of cases  <br> did not  <br> end in <br> prosecution"))
...
figure18.fn <- function(nchart = 18) {
  ...
  legal_assigment <- function(data_subset) {
    ...
    if(mainCountry %in% centralAmerica.ls) {
      ...
      a <- data_subset %>%
      mutate(a2j_land = ifelse(q19_B1 == 1 | q19_B2 == 1 | q19_B3 == 1| q19_B4 == 1, 1, 0),
             a2j_housing = ifelse(q19_C3 == 1, 1, 0),
             a2j_law = ifelse(q19_I1 == 1, 1, 0),
             a2j_money =  ifelse(q19_K1 == 1 | q19_K2 == 1 | q19_K3 == 1 | q19_L1 == 1 | q19_L2 == 1, 1, 0),
             a2j_community = ifelse(q19_E3 == 1, 1, 0)) %>%
      mutate(legal = ifelse(a2j_land == 1 | a2j_housing == 1 | a2j_law == 1 | a2j_money == 1 | a2j_community == 1 | q19_99 == 1, 1, 0))
    } else {
  ...
  if(mainCountry %in% centralAmerica.ls) {
    a2j_a <- a2j %>%
      ...
      rename("Community" = "a2j_community",
             "Housing" = "a2j_housing",
             "Land" = "a2j_land",
  ...
  first_panel <- a2j_b[1:3,] %>%
    aes_function_v2(.) 
    # mutate(value = if_else(value > 0.8 & group %in% "value", 0.75, value),
    #        value = if_else(value < 0.1 & group %in% "empty_value", 0.25, value),
    #        empty_value = if_else(empty_value < 0.1 & group %in% "value", 0.25, value),
    #        empty_value = if_else(empty_value >  0.8 & group %in% "empty_value", 0.75, value))
...
figure18_US.fn <- function(nchart = 18) {
  ...
  data2plot <- data_subset.df %>%
    ...
    filter(year > 2013) %>%
    mutate(Courts = if_else(Courts %in% "NaN", NA_real_, Courts)) %>%
    pivot_longer(!year,
                 names_to  = "category",
                 values_to = "value") %>%
    mutate(value = value*100,
